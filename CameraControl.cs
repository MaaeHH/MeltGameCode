...
public class CameraControl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        /*Vector3 positionToCheck = this.transform.position; // Replace transformToCheck with your Vector3

        if (IsPositionWithinRadius(positionToCheck))
        {
            // The position is within the specified radius
            //Debug.Log("Position is within the radius.");
        }
        else
        {
            // The position is outside the specified radius
            Debug.Log("Position is outside the radius.");
        }*/
    }
    ...
    {
        if (IsPositionWithinRadius(transform.position + targetPosition * speed * Time.deltaTime))
        {
    ...
    {
        // Calculate the distance between the center position and the provided position
        float distance = Vector3.Distance(centerTransform.position, position);

        // Check if the distance is less than or equal to the radius
        if (distance <= radius)
        {
            return true; // The position is within the radius
        }
        else
        {
            return false; // The position is outside the radius
        }
    }
    void OnDrawGizmos()
    {
